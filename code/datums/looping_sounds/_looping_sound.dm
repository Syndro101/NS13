...
/datum/looping_sound/proc/stop(null_parent = FALSE)
	...
	if(!timer_id)
		return
	on_stop()
	deltimer(timer_id, timer_subsystem = SSsound_loops)
	timer_id = null
	loop_started = FALSE
...
/datum/looping_sound/proc/on_start()
	...
	timer_id = addtimer(CALLBACK(src, PROC_REF(start_sound_loop)), start_wait, TIMER_CLIENT_TIME | TIMER_DELETE_ME | TIMER_STOPPABLE, timer_subsystem = SSsound_loops)
